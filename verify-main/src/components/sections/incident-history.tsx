    ...
    <div className="mb-8">
      <div className="flex items-center gap-2 mb-4 pb-2 border-b border-border">
        ...
        <Badge variant="secondary" className="text-xs">
          {t('incidentCount', { count: incidentDay.incidents.length })}
        </Badge>
      </div>
  ...
  return (
    <div className="text-center py-12">
      ...
      <h3 className="text-lg font-semibold text-foreground mb-2">
        {showAllIncidents ? t('noIncidentsDescription') : t('noResolvedIncidents')}
      </h3>
      <p className="text-muted-foreground">
        {showAllIncidents
          ? t('noIncidentsDescription')
          : t('noResolvedDescription')
        }
      </p>
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-foreground">
          {showAllIncidents ? t('allIncidents') : t('title')}
        </h2>
        <div className="flex items-center gap-3">
          {!showAllIncidents && (
            ...
            >
              {t('allIncidentsDetails')}
            </button>
          )}
          <Badge variant="outline" className="text-sm">
            {showAllIncidents ? t('totalAll', { count: incidents.length }) : t('totalResolved', { count: incidents.length })}
          </Badge>
        </div>
      </div>
